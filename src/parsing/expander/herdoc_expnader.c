...
/*   Updated: 2024/08/21 18:28:47 by aghergho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_count_expanded_line(char *line, int *counter)
{
    ...
    while (line[i] && is_symbole(line[i]))
        i++;
...
int ft_counte_herdoc_len(char *line)
{
    ...
    while (line[++i])
    {
        if (is_dollar_sign(line[i]) && line[i + 1] && is_symbole(line[i + 1]))
...
void ft_gen_expanded_herdoc(char **new, char *line)
{
    ...
    while (line[++i])
    {
        if (is_dollar_sign(line[i]) && line[i + 1] && (ft_isalnum(line[i + 1]) || is_dollar_sign(line[i + 1])))
